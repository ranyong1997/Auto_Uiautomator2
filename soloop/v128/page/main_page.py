...
class MainPage(BasePage):
    ...
    def click_contact_details(self):
        self.find_element_and_click(**self.element["contact_details"])
        self.find_element_and_click(**self.element["issue"])
        self.find_element_and_click(**self.element["select"])

    @allure.step("反馈内容")
    def click_feedback_content(self):
        self.driver.click(0.208, 0.249)
        self.driver.send_keys("测试反馈")

    @allure.step("联系方式")
    def input_contact_details(self):
        self.find_element_and_click(**self.element["contact_way"])
