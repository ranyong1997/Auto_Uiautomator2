...
from common.aircv import cv2
from common.aircv.error import TargetNotFoundError, InvalidMatchingMethodError
from common.aircv.keypoint_matching import KAZEMatching, BRISKMatching, AKAZEMatching, ORBMatching
from common.aircv.keypoint_matching_contrib import SIFTMatching, SURFMatching, BRIEFMatching
from common.aircv.settings import Settings as ST
from common.aircv.template_matching import TemplateMatching
from common.aircv.utils import TargetPos
...
class ImageX(object):
    ...
    def _loop_find(self, query, timeout=ST.FIND_TIMEOUT, threshold=None, interval=0.5, intervalfunc=None):
        """
        ...
        """
        logging.info("Try finding: %s", query)
        start_time = time.time()
        while True:
            screen = self._d.screenshot(format='opencv')

            if screen is None:
                logging.warning("Screen is None, may be locked")
            else:
                if threshold:
    ...
    def _try_log_screen(self, screen=None, quality=None, max_size=None):
        """
        ...
        if not os.path.exists(ST.LOG_DIR):
            os.makedirs(ST.LOG_DIR)
        aircv.imwrite(filepath, screen, quality, max_size=max_size)
        return {"screen": filename, "resolution": aircv.get_resolution(screen)}

    def wait(self, imdata, timeout=20.0):
        """ wait until image show up """
        m = self._loop_find(Template(imdata), timeout=timeout)
        return m

    def click(self, imdata, timeout=20.0):
        """
        Args:
        ...
        return self.send_click(res)

    def blank_detection(self, threadhold=0.03):
        """ 空白检测 """
        base_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "aircv", "white.png")
        base = aircv.imread(base_file)
        screen = self._d.screenshot(format='opencv')
        height, width = screen.shape[:2]
        rect = (0, 250, width, height - 150)
        screen_crop = aircv.crop_image(screen, rect)
        # aircv.show(screen_crop)
        base_gray = cv2.cvtColor(base, cv2.COLOR_BGR2GRAY)
        screen_gray = cv2.cvtColor(screen_crop, cv2.COLOR_BGR2GRAY)

        ret, th = cv2.threshold(screen_gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

        hist_base = cv2.calcHist([base_gray], [0], None, [256], (0, 256), accumulate=False)
        hist_screen = cv2.calcHist([th], [0], None, [256], (0, 256), accumulate=False)

        cv2.normalize(hist_base, hist_base, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX)
        cv2.normalize(hist_screen, hist_screen, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX)

        base_test1 = cv2.compareHist(hist_base, hist_screen, 3)
        if base_test1 <= threadhold:
            logging.debug("Screen is blank: {}".format(base_test1))
            return True
        else:
            logging.debug("Screen is not blank: {}".format(base_test1))
            return False
...
class Template(object):
    ...
    def match_in(self, screen):
        ...
        logging.debug("match result: %s", match_result)
        if not match_result:
            return None
    ...
    def _try_match(func, *args, **kwargs):
        logging.debug("try match with %s" % func.__name__)
        try:
            ret = func(*args, **kwargs).find_best_result()
        except aircv.NoModuleError as err:
            logging.debug("'surf'/'sift'/'brief' is in opencv-contrib module. You can use 'tpl'/'kaze'/'brisk'/'akaze'/'orb' in CVSTRATEGY, or reinstall opencv with the contrib module.")
            return None
    ...
    def _resize_image(self, image, screen, resize_method):
        ...
        # 调试代码: 输出调试信息.
        logging.debug("resize: (%s, %s)->(%s, %s), resolution: %s=>%s" % (
